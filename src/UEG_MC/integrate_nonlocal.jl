...
function mc_variables_fixed_extK(mcparam::UEG.ParaMC, alpha::Float64, nT_fixed::Int=3)
    R = Continuous(0.0, 1.0; alpha=alpha)
    Theta = Continuous(0.0, 1π; alpha=alpha)
    Phi = Continuous(0.0, 2π; alpha=alpha)
    K = CompositeVar(R, Theta, Phi)
...
"""Measurement for a single partition (without CTs, fixed order in V)."""
@inline function measure_single_fixed_extK(vars, obs, weights, config)
    obs[1] += weights[1]
    return
end
...
"""Measurement for multiple (N) partitions and fixed external momentum (with CTs, fixed order in ξ)."""
function measure_fixed_extK(vars, obs, weights, config)
    # Measure the weight of each partition
    for o in 1:(config.N)
        obs[o] += weights[o]
    end
...
function integrand_single_fixed_extK(vars, config)
    # We sample internal momentum/times, and external momentum index
    K, T = vars
    R, Theta, Phi = K

    # Unpack userdata
    mcparam, exprtrees, innerLoopNums, prefactors, varK, extK = config.userdata

    # Evaluate the integrand for each partition
    integrand = Vector(undef, config.N)
    for i in 1:(config.N)
        # Fixed external momentum magnitude k = extK (wlog, we place it along the x-axis)
        varK[1, 1] = extK
...
function integrand_full_fixed_extK(vars, config)
    # We sample internal momentum/times, and external momentum index
    K, T = vars
    R, Theta, Phi = K
