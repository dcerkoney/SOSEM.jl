...
function integrate_nonlocal_with_ct(
    ...
    # List of expression tree roots and external times
    rootlist = [et.root for et in exprtrees]
    extTs =
        [[exprtrees[i].note.object[r] for r in roots] for (i, roots) in enumerate(rootlist)]
