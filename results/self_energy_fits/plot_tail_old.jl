...
function main()
    ...
    basic_param_bare = Parameter.rydbergUnit(1 / param.beta, param.rs, param.dim)
    sigma_tau_dynamic, sigma_tau_instant = SelfEnergy.G0W0(basic_param_bare)
    ...
    # NOTE: Here we need to be careful to generate Σ_G0W0 for the *bare* theory, i.e.,
    #       to use an ElectronGas.Parameter object where Λs (mass2) is zero!
    basic_param2_bare = Parameter.rydbergUnit(1 / param.beta, 2.0, param.dim)

    # Get the G0W0 self-energy and corresponding DLR grid from the ElectronGas package
    sigma2_tau_dynamic, sigma2_tau_instant = SelfEnergy.G0W0(basic_param2_bare)
    ...
    plt.tight_layout()
    fig.savefig(
        "results/self_energy_fits/sigma_g0w0_tail_comparison_" *
