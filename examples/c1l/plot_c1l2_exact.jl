...
ls = LinRange(log(1e-3), log(2.0), 100)
lls = exp.(ls)

fig, ax = plt.subplots()
ax.axhline(0.0; linestyle="--", linewidth=1, color="k")
ax.plot(lls, c1l2_over_eTF_vlambda_vlambda.(lls); label="\$V_\\lambda V_\\lambda\$")
ax.plot(lls, c1l2_over_eTF_v_vlambda.(lls); label="\$V V_\\lambda\$")
ax.set_xlim(0.0, 2.0)
ax.set_ylim(-4.0, 4.0)
ax.legend(loc="best")
