...
function main()
    ...
    filenames = [
        "results/data/c1d_n=$(order)_rs=$(rs)_" *
    ...
    for partitions in partitions_list
        for p in partitions
            ...
            ax.plot(
                ...
                # color="C$next_color",
                label="\$\\widetilde{C}^{(1d)}_{$p}\$",
                # label="\$\\widetilde{C}^{(1d)}_{$p}\$ ($solver)",
    ...
    if max_order_plot ≥ 4
        c1d4_means = Measurements.value.(c1d[3])
        c1d4_errs = Measurements.uncertainty.(c1d[3])
        ax.plot(
            k_kf_grid,
            c1d4_means,
            "-";
    ...
    fig.savefig(
        ...
        "neval=$(neval)_$(intn_str)$(solver)_$(ct_string)_ct_cancellation_fo.pdf",
    )
    plt.close("all")
    return
end
