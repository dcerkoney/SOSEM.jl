...
"""
Aggregate the measurements for C⁽¹ᵇ⁾⁰ up to order N for nmin ≤ N ≤ nmax.
Assumes the input data has already been merged by interaction order and, 
if applicable, reexpanded in μ.
"""
function aggregate_results_c1N(merged_data; nmax, nmin=2)
    c1N = Dict()
    for n in nmin:nmax
        c1N[n] = zero(merged_data[(n, 0)])
        println(n)
...
function main()
    ...
    # Aggregate the full results for C⁽¹ᵇ⁾
    c1bL0N = aggregate_results_c1N(merged_data; nmax=max_order_plot, nmin=min_order)
    ...
    for N in min_order:max_order_plot
        ...
        means = 2 * Measurements.value.(c1bL0N[N])
        stdevs = 2 * Measurements.uncertainty.(c1bL0N[N])
    ...
    ax.set_xlabel("\$k / k_F\$")
    ax.set_ylabel(
        "\$C^{(1b)0}_{N}(\\mathbf{k}) \\,/\\, {\\epsilon}^{\\hspace{0.1em}2}_{\\mathrm{TF}}\$",
    )
    yoffset = -0.15
    # ax.text(
    #     1.75,
