...
function main()
    ...
    if !isnothing(res) && plot
        ...
        if compare_bare
            ...
            c1bL0_quad_dimless = sosem_quad.get("bare_b") / eTF_quad^2 / 2  # C⁽¹ᵇ⁾ᴸ = C⁽¹ᵇ⁾ / 2
            ax.plot(
                k_kf_grid_quad,
                c1bL0_quad_dimless,
                "k";
        ...
        ax.set_xlabel("\$k / k_F\$")
        ax.set_ylabel(
            "\$C^{(1b)L0}_{\\mathcal{P}}(\\mathbf{k}) \\,/\\, E^{2}_{\\mathrm{TF}}\$",
        )
        ax.set_xlim(minimum(k_kf_grid), maximum(k_kf_grid))
        plt.tight_layout()
        fig.savefig(
            "results/c1b0/n=$(param.order)/c1bL0_n=$(param.order)_rs=$(param.rs)_" *
            "beta_ef=$(param.beta)_lambda=$(param.mass2)_" *
