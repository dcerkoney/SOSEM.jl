...
function main()
    ...
    for (i, order) in enumerate(orders)
        ...
        if i == 1
            ...
            bare_quad = np.load("results/data/soms_rs=$(rs_quad)_beta_ef=200.0.npz")
            c1b0_bare_quad = bare_quad.get("bare_b") / eTF_quad^2
            ax.plot(k_kf_grid_quad, c1b0_bare_quad, "k"; label="\$LO\$ (quad)")

            # RPA+FL results for class (b) moment
            rpa_fl = np.load("results/data/soms_rs=$(rs_quad)_beta_ef=200.0.npz")
            c1b_rpa_fl = rpa_fl.get("rpa+fl_b") / eTF_quad^2
            c1b_rpa_fl_err = rpa_fl.get("rpa+fl_b_err") / eTF_quad^2
    ...
    ax.set_xlabel("\$k / k_F\$")
    ax.set_ylabel(
        "\$C^{(1b)0}_n(\\mathbf{k}) \\,/\\, {\\epsilon}^{\\hspace{0.1em}2}_{\\mathrm{TF}}\$",
    )
