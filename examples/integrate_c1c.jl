...
function main()
    ...
    settings =
        DiagGen.Settings(; observable=DiagGen.c1c, n_order=2, verbosity=DiagGen.info)

    # UEG parameters for MC integration
    # NOTE: To match units, we specify (beta / EF) = 2 * (heg_soms.beta)
    params =
        ParaMC(; order=settings.n_order, rs=2.0, isDynamic=false, beta=40.0, mass2=0.0001)
    if is_main_thread
    ...
    if !isempty(means) && plot
        # Save the result
        np.savez(
            "results/data/c1c_rs=$(params.rs)_beta_ef=$(params.beta)_neval=$(maxeval)";
        ...
        plt.tight_layout()
        fig.savefig(
            "results/c1c_n=$(params.order)_rs=$(params.rs)_" *
