...
function process_mass_ratio(datatuple, isSave; idk=1)
    ...
    signstrs = ["++", "--", "+-", "-+"]
    for (signs, signstr) in zip(sign_combinations, signstrs)
        ...
        if isSave
            ...
            jldopen(
                "data/mass_ratio_from_sigma_gridtest_$(signstr).jld2",
                "a+";
                compress=true,
            ) do f
            ...
            jldopen("inverse_zfactor_gridtest_$(signstr).jld2", "a+"; compress=true) do f
                key = "$(UEG.short(para))"
                if haskey(f, key)
                    @warn("replacing existing data for $key")
                    delete!(f, key)
            ...
            jldopen("zfactor_approx_gridtest_$(signstr).jld2", "a+"; compress=true) do f
                key = "$(UEG.short(para))"
                if haskey(f, key)
                    @warn("replacing existing data for $key")
                    delete!(f, key)
        ...
        for key in keys(f)
            if UEG.paraid(f[key][1]) == UEG.paraid(para)
                ...
                for idk in idks
                    ...
                    process_mass_ratio(f[key], isSave; idk=idk)
                end
                println("done!")
            end
        end
