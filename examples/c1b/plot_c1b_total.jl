...
"""
function aggregate_results_c1bN(c1b; nmax, nmin=2, renorm_mu=false)
    c1b_total = Dict()
    if renorm_mu
        # merged data is an ordered vector of data at each order nmin ≤ n ≤ nmax
        c1b_total =
            Dict(zip(nmin:nmax, accumulate(+, c1b[i] for i in eachindex(nmin:nmax))))
    else
        # merged data is a Dict of interaction-merged partitions P
        for n in nmin:nmax
            c1b_total[n] = zero(c1b[(n, 0)])
            println(n)
            for (p, meas) in c1b
                # if p[1] <= n
                if sum(p) <= n
...
function main()
    ...
    if renorm_mu
        if renorm_mu_lo_ex && max_order_plot == 3
            ...
            c1b2 = merged_data[(2, 0)]
            c1b3 = merged_data[(3, 0)] + δμ1 * merged_data[(2, 1)]
            c1b = [c1b2, c1b3]
        else
            ...
            if !renorm_mu_lo_ex && max_order >= 3
                ...
                c1b3 = c1b[1] + c1b[2]
                stdscores = stdscore.(c1b3, c1b3_manual)
    ...
    if renorm_mu_lo_ex
        c1b_total = Dict(2 => c1b2, 3 => c1b2 + c1b3)
    else
        c1b_total = aggregate_results_c1bN(
            c1b;
            nmin=min_order,
            nmax=max_order_plot,
            renorm_mu=renorm_mu,
    ...
    for N in min_order:max_order_plot
        ...
        means = 2 * Measurements.value.(c1b_total[N])
        stdevs = 2 * Measurements.uncertainty.(c1b_total[N])
        ...
        if !renorm_mu_lo_ex && max_order <= 3 && N == 3
            ax.plot(
                k_kf_grid,
                Measurements.value.(c1b3_manual);
                color="r",
                linestyle="--",
