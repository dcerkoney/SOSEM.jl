...
function main()
    ...
    neval = 1e10  # a reduced neval is sufficient to test this single observable
    ...
    if !isnothing(res)
        savename =
            "results/data/c1bL_k=$(extK)_n=$(param.order)_rs=$(param.rs)_" *
    ...
    if !isnothing(res)
        ...
        jldopen("$savename.jld2", "a+"; compress=true) do f
            key = "c1bL_k=$(extK)_n_min=$(settings.min_order)_n_max=$(settings.max_order)_neval=$(neval)"
            if haskey(f, key)
                @warn("replacing existing data for $key")
                delete!(f, key)
            end
