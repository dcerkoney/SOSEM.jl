...
function main()
    ...
    fig, ax = plt.subplots()
    nevals = [neval_rest, neval_rest, neval_c1d]
    obsnames = ["c1b_total", "c1c", "c1d"]
    obsprefixes = [["\\delta C^{(1b0)}_{$plot_order}", "C^{(1b)}_{$plot_order}"], "C^{(1c)}_{$plot_order}", "C^{(1d)}_{$plot_order}"]
    next_color = 0
    for (i, obsname) in enumerate(obsnames)
        # Load the data for this observable
        f = jldopen("$filename.jld2", "r")
        if obsname == "c1b_total"
            meas0 = f["c1b0/N=$plot_order/neval=$(nevals[i])/meas"]
            meas = f["c1b/N=$plot_order/neval=$(nevals[i])/meas"]
            meas_rpa = f["c1b/RPA/neval=$(1e7)/meas"]
            meas_rpa_fl = f["c1b/RPA+FL/neval=$(1e7)/meas"]
            param = f["c1b/N=$plot_order/neval=$(nevals[i])/param"]
            kgrid = f["c1b/N=$plot_order/neval=$(nevals[i])/kgrid"]
        else
        ...
        if obsname == "c1b_total"
            ...
            ax.plot(k_kf_grid, c1b_rpa, "k"; linestyle="--", label="\$C^{(1b)}_{\\mathrm{RPA}}\$ (vegas)")
            ax.fill_between(
                k_kf_grid,
                (c1b_rpa - c1b_rpa_err),
                (c1b_rpa + c1b_rpa_err);
            ...
            ax.plot(k_kf_grid, c1b_rpa_fl, "k"; label="\$C^{(1b)}_{\\mathrm{RPA}+\\mathrm{FL}}\$ (vegas)")
            ax.fill_between(
                k_kf_grid,
        ...
        if obsname == "c1b_total"
            ...
            for (i, obsprefix) in enumerate(obsprefixes[i])
                ax.plot(
                    k_kf_grid,
                    means_list[i],
                    marker;
    ...
    ax.text(
        xloc,
        yloc,
        "\$N = $plot_order,\\, r_s = 1,\\, \\beta \\hspace{0.1em} \\epsilon_F = 200,\$";
        fontsize=14,
    )
    ax.text(
        xloc,
