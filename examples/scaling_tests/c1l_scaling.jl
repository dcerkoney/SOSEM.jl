...
function main()
    ...
    if !isempty(results)
        savename =
            "results/data/c1l_scaling_nevals=$(nevals)_n=$(param.order)_rs=$(param.rs)_" *
            "beta_ef=$(param.beta)_lambda=$(param.mass2)_$(solver)$(ct_string)"
        jldopen("$savename.jld2", "a+"; compress=true) do f
            ...
            return f[key] = (nevals, orders, param, partitions, results)
        end
    end
end
