...
function integrate_polarization_with_ct(
    ...
    obs = repeat([zeros(n_kgrid, n_tgrid)], length(dof))  # observable for each partition
    ...
    prefactors = mcparam.spin * phase_factors
...
function main()
    ...
    if !isnothing(res)
        savename =
            "results/data/polarization_n=$(param.order)_rs=$(param.rs)_beta_ef=$(param.beta)_" *
